--This is nothing more than a port of DST's spellcaster component
...
local BetterSpellCaster = Class(function(self, inst)
    self.inst = inst
...
function BetterSpellCaster:OnRemoveFromEntity()
    self.inst:RemoveTag("castfrominventory")
    self.inst:RemoveTag("castontargets")
    self.inst:RemoveTag("castonrecipes")
    self.inst:RemoveTag("castonlocomotors")
...
function BetterSpellCaster:SetSpellFn(fn)
    self.spell = fn
end

function BetterSpellCaster:SetOnSpellCastFn(fn)
    self.onspellcast = fn
end

function BetterSpellCaster:CastSpell(target, pos, doer)
    if self.spell ~= nil then
...
function BetterSpellCaster:SetCanCastFn(fn)
    self.can_cast_fn = fn
end

function BetterSpellCaster:CanCast(doer, target, pos)
    if self.spell == nil then
        return false
    elseif target == nil then
        ...
        if self.canuseonpoint then
            ...
            --if tile ~= GROUND.IMPASSABLE and not ground.Map:IsWater(tile)
            return tile ~= GROUND.IMPASSABLE and not ground.Map:IsWater(tile) 
			--TheWorld.Map:IsAboveGroundAtPoint(px, py, pz, self.canuseonpoint_water) and not TheWorld.Map:IsGroundTargetBlocked(pos)
...
function BetterSpellCaster:CollectPointActions(doer, pos, actions, right)
	if right then
        local cast_on_water = self.canuseonpointwater
        if self.canuseonpoint then
            local px, py, pz = pos:Get()
			local ground = GetWorld()
			local tile = ground.Map:GetTileAtPoint(px, py, pz)
            if tile ~= GROUND.IMPASSABLE and not ground.Map:IsWater(tile) and not doer:HasTag("steeringboat") then
				table.insert(actions, ACTIONS.DSTCASTSPELL)
            end
...
function BetterSpellCaster:CollectEquippedActions(doer, target, actions, right)
	if right and not target:HasTag("nomagic") and (
        (target.components.locomotor and self.canonlyuseonlocomotors) or
		(self.canuseontargets and target.components.locomotor == nil and not self.canonlyuseonlocomotors) or
        (self.canuseonrecipes and AllRecipes[target.prefab] ~= nil and not FunctionOrValue(AllRecipes[target.prefab].no_deconstruction, target)) or
        --[[(target.components.locomotor and (
            self.canonlyuseonlocomotors --or
            --(self.canuseonlocomotorspvp and (target == doer or TheNet:GetPVPEnabled() or not (target:HasTag("player") and doer:HasTag("player"))))
        )) or]]
        (self.canuseonworkable and (target:HasTag("CHOP_workable") or target:HasTag("DIG_workable") or target:HasTag("HAMMER_workable") or target:HasTag("MINE_workable"))) or
        (self.canuseoncombat and doer.components.combat:CanTarget(target))
        ) then
        table.insert(actions, ACTIONS.DSTCASTSPELL)
    end
end

function BetterSpellCaster:CollectInventoryActions(doer, actions)
	print(self.inst)
	print(doer.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS))
	if self:CanCast(doer) and self.canusefrominventory 
		and doer.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS)
		and doer.components.inventory:GetEquippedItem(EQUIPSLOTS.HANDS).components.betterspellcaster == self then
		table.insert(actions, ACTIONS.DSTCASTSPELL)
	end
end

return BetterSpellCaster
