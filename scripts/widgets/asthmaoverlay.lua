...
local AsthmaOverlay = Class(Widget, function(self, inst, anchor_x, anchor_y)
	...
	self.title = self:AddChild(Text(BODYTEXTFONT, 33, "Asthma Risk: ", UICOLOURS.WHITE))
	...
	inst:ListenForEvent("asthmavaldirty", function(inst, data)
		self.riskval = inst.replica.asthma:GetAsthmaVal()
	end)
 ...
 function AsthmaOverlay:OnUpdate(dt)
	...
	elseif self.riskval >= 900 and self.riskval < 1800 and self.risk:GetString() ~= WARNING_MED then
		...
		self.risk:SetColour(255, 255, 0, 1)
